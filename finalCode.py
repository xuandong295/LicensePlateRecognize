...
def CallAPIUploadData(FrontImageLink, BackImageLink, LicensePlateNumber, ParkingAreaId, TimeIn, TimeOut, Status):
    ID = uuid.uuid4()
    ...
    data = data.format(ID=ID, FrontImageLink=FrontImageLink, BackImageLink=BackImageLink,
                       LicensePlateNumber=LicensePlateNumber, ParkingAreaId=ParkingAreaId, TimeIn=TimeIn, TimeOut=TimeOut,
                       Status=Status)
    print(data)
    ...
    if handshake_arduino(arduino, print_handshake_message=True) == "1":
        ...
        # Capture frame-by-frame
            ...
            if len(final_string)>=7:
                ...
                # CallAPIUploadData(front_image_link, front_image_link, final_string, "633b4557-7cad-4c18-94cd-e939dd0285b6", int(time.mktime(datetime.utcnow().timetuple())), 0, 0)
                lock = False
                break;
